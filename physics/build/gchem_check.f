...
C     GCHEM cannot (yet!) run cfc and dic together
      ...
      IF ( useBIMAP .AND. useCFC ) THEN
       WRITE(msgBuf,'(A)') 'useBIMAP and useCFC cannot both be .TRUE.'
       CALL PRINT_ERROR( msgBuf , myThid )
       STOP 'ABNORMAL END: S/R GCHEM_CHECK'
      ENDIF

      IF ( useFGM .AND. useCFC ) THEN
       WRITE(msgBuf,'(A)') 'useFGM and useCFC cannot both be .TRUE.'
       CALL PRINT_ERROR( msgBuf , myThid )
       STOP 'ABNORMAL END: S/R GCHEM_CHECK'
      ENDIF
